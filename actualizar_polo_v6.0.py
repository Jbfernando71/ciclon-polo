...
def main():
    print("=" * 72)
    print(" ACTUALIZADOR CICLÓN POLO V6.2")
    ...
    # La redacción del SMN puede variar entre avisos.
    patrones_huracan = [
        r"HURAC[AÁ]N\s+POLO(?:\s+AHORA\s+COMO)?\s+(?:DE\s+)?CATEGOR[IÍ]A\s+([1-5])",
        r"POLO.{0,80}?HURAC[AÁ]N(?:\s+DE)?\s+CATEGOR[IÍ]A\s+([1-5])",
        r"HURAC[AÁ]N(?:\s+DE)?\s+CATEGOR[IÍ]A\s+([1-5]).{0,80}?POLO",
    ]
    categoria = None
    for patron in patrones_huracan:
        mm = re.search(patron, t, re.I | re.S)
        if mm:
            categoria = mm.group(1)
            break

    if categoria:
        clasificacion = f"Huracán categoría {categoria}"
    elif re.search(r"POLO.{0,80}?TORMENTA\s+TROPICAL|TORMENTA\s+TROPICAL.{0,80}?POLO", t, re.I | re.S):
        clasificacion = "Tormenta tropical"
    elif re.search(r"POLO.{0,80}?DEPRESI[ÓO]N\s+TROPICAL|DEPRESI[ÓO]N\s+TROPICAL.{0,80}?POLO", t, re.I | re.S):
